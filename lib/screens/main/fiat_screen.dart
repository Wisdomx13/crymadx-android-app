...
/// Fiat On-Ramp Screen with demo workflow
...
class _FiatScreenState extends State<FiatScreen> {
  ...
  final List<String> _fiats = ['USD', 'EUR', 'GBP', 'CAD', 'AUD'];
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(12),
                        border: Border.all(
                          color: isSelected ? provider['color'] as Color : borderColor,
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Row(
                                  children: [
                                    Text(provider['name'] as String, style: TextStyle(color: textColor, fontSize: 15, fontWeight: FontWeight.w600)),
                                ...
                                Text('Fee: ${provider['fee']} | ${provider['limit']}', style: TextStyle(color: Colors.grey[600], fontSize: 11)),
                                const SizedBox(height: 4),
                                Row(
                                  children: (provider['methods'] as List<String>).map((method) {
                                    return Container(
                          ...
                          if (isSelected) Icon(Icons.check_circle, color: provider['color'] as Color, size: 24),
                        ],
                      ),
                    ),
                  );
  ...
  String _calculateReceiveAmount() {
    ...
    final crypto = _cryptos.firstWhere((c) => c['symbol'] == _selectedCrypto);
    final price = crypto['price'] as double;
    final fee = double.parse(_providers[_selectedProvider]['fee'].replaceAll('%', '')) / 100;
  ...
  Widget _buildReviewStep(Map<String, dynamic> provider, Color textColor, Color cardColor) {
    final amount = double.tryParse(_amount) ?? 100;
    final fee = double.parse(provider['fee'].replaceAll('%', '')) / 100;
    final crypto = _cryptos.firstWhere((c) => c['symbol'] == _selectedCrypto);
          ...
          child: Column(
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Provider Fee (${provider['fee']})', style: TextStyle(color: Colors.grey[600], fontSize: 14)),
                  Text(
            ...
            children: [
              Container(
                ...
                child: Center(
                  child: Text(
                    provider['logo'] as String,
                    style: TextStyle(color: provider['color'] as Color, fontWeight: FontWeight.w700),
                  ),
                ),
              ),
  ...
  Widget _buildVerifyStep(Map<String, dynamic> provider, Color textColor, Color cardColor, bool isDark) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
